...
export class srController {
    ...
    public RefundAmount = async(req: Request, res: Response): Promise<Response | undefined> => {
        const token = req.headers.authorization || req.header('auth');
        if(token) {
            jwt.verify(token, process.env.SECRET_KEY!, (error, user: any) => {
                if(error) {
                            ...
                            return this.srService
                              ...
                              .then(async (service) => {
                                if(service && service?.Status === 2) {
                                    const amount = await this.srService.calculateAmount(service.TotalCost, req.body.Amount);
                                    if(+service.TotalCost > +amount) {
                                        return this.srService
                                          .updateRefund(+req.params.ServiceId, +amount)
                                          .then((refund) => {
                                              if(refund) {
                                                return res.status(200).json({
                                                    PaidAmount: service.TotalCost + " €",
                                                    RefundedAmount: amount + " €",
                                                    InBalanceAmount: ((+service.TotalCost) - (+amount)) + " €"
                                                });
                                              }
                                              else {
                                        ...
                                        return res.status(400).json("Refund Amount must be less than Paid Amount!");
                                    }
                                }
                                else {
                                    return res.status(404).json("Service not found or Service is not completed!");
                                }
                            })
                            .catch((error: Error) => {
                              return res.status(500).json({ error: error });

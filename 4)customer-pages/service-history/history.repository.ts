...
export class HistoryRepository {
    ...
    public async getAllPastRequest(UserId: number): Promise<ServiceRequest[]> {
        return db.ServiceRequest.findAll({ where: { UserId: UserId, Status: {[Op.or] : [2,3,5]} }, include: ['HelperRequest', 'ServiceRating'] });
    }

    public async getServiceById(ServiceId: number): Promise<ServiceRequest | null> {
        return db.ServiceRequest.findOne({ where: { ServiceId: ServiceId, Status: {[Op.or] : [2,3,5]} }, include: ['HelperRequest', 'ServiceRequestAddress', 'ExtraService', 'ServiceRating'] });
    }

...
export class SpHistoryRepository {
    ...
    public async getAllCompletedRequest(ZipCode: number | undefined): Promise<ServiceRequest[]> {
        return db.ServiceRequest.findAll({ where: {ZipCode: ZipCode, Status: 2}, include: ['UserRequest', 'ServiceRequestAddress'] });
    }

    public async getServiceById(ServiceId: number, ZipCode: number | undefined): Promise<ServiceRequest | null> {
        return db.ServiceRequest.findOne({ where: { ServiceId: ServiceId, ZipCode: ZipCode, Status: 2 }, include: ['UserRequest', 'ServiceRequestAddress', 'ExtraService'] });
    }

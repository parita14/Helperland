                                    ...
                                    if (doMatch) {
                                        ...
                                        if ((user === null || user === void 0 ? void 0 : user.UserTypeId) === 1) {
                                            return [2 /*return*/, res
                                                    ...
                                                    .json({ message: "Customer named " + user.FirstName + " is Logged In Successfully", token: token })];
                                        }
                                            ...
                                            if (user.IsApproved === true) {
                                                return [2 /*return*/, res
                                                        .status(200)
                                                        .cookie("token", token, { httpOnly: true, expires: new Date(Date.now() + 600000) })
                                                        .json({ message: "Service Provider named " + user.FirstName + " is Logged In Successfully", token: token })];
                                            }
                                            else {
                                                return [2 /*return*/, res.status(400).json("You can't login, You are not approved by Admin!")];
                                            }
                                        }
                                        else {
                                            return [2 /*return*/, res
                                                    .status(200)
                                                    .cookie("token", token, { httpOnly: true, expires: new Date(Date.now() + 600000) })
                                                    .json({ message: "Admin named " + user.FirstName + " is Logged In Successfully", token: token })];
                                        }
                                    }
                                    else {
                                        return [2 /*return*/, res.status(400).json("Invalid Username or Password!")];
                                    }
                                    return [2 /*return*/];
                                });
                            }); })
